// ============================================================================
// 🏆 🇯🇴 TEAM1_TOURNAMENT_EXTREME++ (FINAL) - ES5 SAFE
// الهدف: أعلى نسبة لاعبين أردنيين + بنق منخفض/ثابت (PAC-only)
// ---------------------------------------------------------------------------
// DIRECT: YouTube/GitHub فقط
// Non-PUBG: DIRECT
// Lobby/Search: JO_WIDE classify + JO lock (first 180s, CORE only)
// Match: 10012 ONLY + "MAX JO ONLY" strict check
// If MATCH IPv4 is NOT in JO_MATCH_STRICT => SOFTBLOCK during 15 minutes
...
var LOCK_MS  = 180000;   // country lock window (first 3 minutes)
...
function pickSearchProxy(ip){
  // lock decision only in first 3 minutes (implicitly: CORE traffic triggers it)
...
function FindProxyForURL(url, host){
  ...
  if (hasAny(h, MATCH_HINTS)) {
    // once match is set, never change (stability)
    if (ST_MATCH) return ST_MATCH;

    var ipm = dnsIP(host);

    // If IPv4 known AND not in strict match ranges => SOFTBLOCK during tournament window
    if (isIPv4(ipm) && !inAny(ipm, JO_MATCH_STRICT)) {
      if (inWindow(TOURNAMENT_MS)) return SOFTBLOCK;
      // after window allow entry to avoid infinite loop
      return sticky("MATCH", MATCH_ONLY);
    }

    // IPv6/unknown OR JO => allow stable match
    return sticky("MATCH", MATCH_ONLY);
  }
  ...
  if (hasAny(h, SEARCH_CORE) || hasAny(h, SEARCH_NOISE)) {
    ...
    // NOISE: keep light to avoid churn
    if (hasAny(h, SEARCH_NOISE) && !hasAny(h, SEARCH_CORE)) {
      ST_SEARCH = SEARCH_NON;
      return ST_SEARCH;
    }
