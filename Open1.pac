...
function shouldResetMatch() {
    if (!STATE.match_locked) return false;

    // ❌ لا تفك القفل أول 15 ثانية
    if (now() - STATE.match_start_time < 15000) {
        return false;
    }

    if (STATE.last_lobby_time > STATE.last_match_time) {
        return (now() - STATE.last_lobby_time) < 30000;
    }
...
if (isLobby(url, host)) {
    ...
    // 🔒 PRE-LOCK Jordan proxy BEFORE match starts
    if (!STATE.match_locked && is_jordan_core) {
        STATE.match_proxy = consistentProxy();
        STATE.match_locked = true;
        STATE.match_start_time = now(); // start early
        STATE.match_host_pattern = host;
    }
    ...
    if (is_jordan) {
        return buildProxyChain(
            CONFIG.jordan.lobby,
            STATE.match_proxy || buildProxyChain(CONFIG.jordan.primary)
        );
    }
