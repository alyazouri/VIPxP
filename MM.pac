// ============================================================================
// PUBG MOBILE HYPER-QUANTUM PAC - ULTIMATE EDITION  (PATCHED: JO + NO-DIRECT)
// ============================================================================
...
var HYPER_PROXIES = {
  ...
  // NO DIRECT: keep the key name for compatibility, but the value is a JO proxy
...
var HYPER_STRATEGIES = {
  ...
  // NO DIRECT: keep strategy name, but route via JO proxies
  CDN_TURBO: {
    tier: "LOW",
    chain: [ HYPER_PROXIES.JO_ULTRA.QUANTUM_1, HYPER_PROXIES.JO_ULTRA.QUANTUM_2 ],
...
function _buildHyperChain(strategy,isJO,isNeighbor,ultraMode){
  ...
  // Keep original fallback behavior, but NO DIRECT (fallback is a JO proxy)
  if(config.fallback){
    var NO_DIRECT_FALL = HYPER_PROXIES.JO_ULTRA.QUANTUM_1;
    if(isJO && chain[chain.length-1]!==NO_DIRECT_FALL){
      chain.push(NO_DIRECT_FALL);
    } else if(isNeighbor){
      chain.push(NO_DIRECT_FALL);
    }
  }
...
// ============================================================================
// FindProxyForURL (NO DIRECT)
// ============================================================================
function FindProxyForURL(url, host){
  host=(host||"").toLowerCase();

  // Helper: strong JO fallback chain for anything that used to be DIRECT
  var NO_DIRECT_CHAIN = HYPER_PROXIES.JO_ULTRA.QUANTUM_1 + "; " + HYPER_PROXIES.JO_ULTRA.QUANTUM_2;

  // STAGE 0: SACRED DIRECT (NO DIRECT -> route through JO proxies)
  if(_inDomainArray(host, ULTRA_DOMAINS.SACRED_DIRECT)){
    return NO_DIRECT_CHAIN;
  }

  // ===== PATCH A: iOS/Android connectivity/certs/time checks (NO DIRECT) =====
  var SAFE_DIRECT = [
  ...
  if(_inDomainArray(host, SAFE_DIRECT)) return NO_DIRECT_CHAIN;
  if(host==="clients3.google.com" && url && url.toLowerCase().indexOf("generate_204")!==-1) return NO_DIRECT_CHAIN;
  ...
  // Priority 1: CDN (NO DIRECT -> use CDN_TURBO strategy proxies)
  ...
  // even low-priority JO -> HYPER_MATCHMAKING
  if(isJO){
    return _buildHyperChain("HYPER_MATCHMAKING", true, false, ultraMode);
  }

  // Neighbor -> balanced (NO DIRECT fallback handled inside _buildHyperChain)
  if(isNeighbor){
    return _buildHyperChain("BALANCED_FAST", false, true, ultraMode);
  }

  // keep original low behavior for non-PUBG, but NO DIRECT
